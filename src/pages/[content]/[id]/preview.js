...
export const getStaticPaths = async () => {
    const contents = await api.content.data.getAllByStatus(contentsStatus.draft);
...
export const getStaticProps = async ({ params }) => {
    ...
    const post = (apiPost || {}).state === contentsStatus.draft ? apiPost : {};

    const lastPosts = await api.content.data.getLastPostsByCategoryID(content, contentsConfig.limit, contentsStatus.draft, post.id);

    return { props: { post, posts: lastPosts, content }, revalidate: 1 };
}
